...
fopValidate: function(fop) {
  ...
  switch (fop) {
    ...
    case 'resize':
      ...
      var width_ok = (/^[1-9][0-9]*$/).test(w);
      var height_ok = (/^[1-9][0-9]*$/).test(h);

//      if (width_ok && h == '') {
//        var img = imce.fileGet(imce.vars.prvfid);
//        h = Math.round(w * img.height / img.width);
//        imce.el('edit-height').value = h;
//        height_ok = true;
//      }
//      else if (height_ok && w == '') {
//        var img = imce.fileGet(imce.vars.prvfid);
//        w = Math.round(h * img.width / img.height);
//        imce.el('edit-width').value = w;
//        width_ok = true;
//      }

      if (!width_ok || !height_ok || (maxW && (maxW < w*1 || maxH < h*1))) {
        console.log('hi');
